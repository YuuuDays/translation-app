...
**GPU(cuda)での文字起こし高速化と、VRAM 8GB環境での現実的なトレードオフ**: CPU(`small`・int8)は1セグメントあたり約3.4〜3.8秒かかり、Ollama翻訳(約0.7〜1秒)より遥かに遅いボトルネックだった。`device: "cuda"`・`compute_type: "float16"`に切り替えると2回目以降0.26秒まで短縮できる(実測、RTX 4060/VRAM 8GB)。ただしこのマシンはVRAM 8GBに対しブラウザ・Discord・CLIP STUDIO PAINT等の常駐アプリも同じGPUを使っており、faster-whisper(GPU)とOllama(gemma2:9b)を同時にGPUへ載せると空きがほぼ無くなり(実測125MB)、Ollama側が翻訳のたびに失敗するようになった。優先すべきは翻訳の安定動作のため、**現在はfaster-whisperを`device: "cpu"`・`compute_type: "int8"`に戻し、VRAMをOllama専用にしている**(空き828MBまで回復、`ollama ps`で「100% GPU」を維持できることを確認)。文字起こしがCPUの分遅くなるトレードオフを受け入れた形。GPUに余裕のある環境(VRAM 8GB専有できる、または12GB以上ある等)では、`MainWindow_Loaded`内の`FasterWhisperClient.StartAsync`呼び出しを`device: "cuda"`・`computeType: "float16"`に戻せば文字起こしが速くなる。CUDA推論に必要な`cublas64_12.dll`/`cudnn64_9.dll`は`nvidia-cublas-cu12`/`nvidia-cudnn-cu12`をpipでvenvに入れる形で用意済み(`requirements.txt`)。これらのDLLはvenv内(`site-packages/nvidia/{cublas,cudnn}/bin`)に入るだけでPATHには通らないため、`ScriptLocator.FindCudaLibraryDirs()`で探して`FasterWhisperClient.StartAsync`がPythonプロセス起動時のPATH環境変数に追加している(CPU運用時は何もしない)。
...
**しばらく使うと「翻訳失敗」が出た問題(VRAM不足)**: faster-whisperをGPU化した後、gemma2:9b(既定コンテキスト長4096)と同時にGPUへ載せるとVRAM(8GB)がほぼ埋まり(`nvidia-smi`実測で空き138MB)、`ollama ps`で見るとgemma2:9bが「20%/80% CPU/GPU」のように一部CPUへ追い出されていた。これにより翻訳が極端に遅くなったり失敗したりしていた。

対策として、まず翻訳プロンプトは短いので4096トークンものコンテキストは不要と判断し`OllamaOptions.NumCtx`を1024に絞った(モデルサイズ7.1GB→5.7GB)。それでもこの環境はブラウザ・Discord・CLIP STUDIO PAINTなど他の常駐アプリも同じGPUを使っており、faster-whisperもGPUに載せたままだと空きは125MB程度とまだ厳しかったため、最終的に**faster-whisperをCPU(`device: "cpu"`)に戻し、GPUはOllama専用にする方針にした**(空きVRAM 828MBまで回復、`ollama ps`で常に「100% GPU」を維持できることを実機で確認)。翻訳の安定動作を優先し、文字起こしが多少遅くなるトレードオフを受け入れている。
